...
# Displays welcome message and instructions for the game of nim
def write_welcome_message():
    ...
    "              If the opponent selects the final stone, you are the winner."):
        ...
        welcome.write(sentence, font = ('Impact',15,'normal'))
        initial_y = initial_y - 25
    welcome.color("green")
    welcome.goto(-170,60)
    welcome.write("Press 'space' for the computer's move.",font = ('Impact',15,'normal'))
...
def drawStones():
    ...
    stones.color("darkblue")
    stones.shape(TURLE_SHAPE)
    ...
    for i in range(random.randrange(MIN_PILE, MAX_PILES + 1)):
        pile = [DrawPileStorePosition(stones,stone_x, stone_y, changeColor(pile_colors))]
        coordinates = coordinates + pile
        stone_y = stone_y + 60
        pile_colors = pile_colors + 1
    return coordinates
...
def NimSum(row1, row2, row3, row4, row5):
    ...
    #Adds zero if even number and 1 is number is odd
    value = ""
...
def removeStone(x,y,color):
    ...
    remove_stones.shape(TURLE_SHAPE)
...
def gamestateToGraphics():
    ...
    logicToVisual.shape(TURLE_SHAPE)
    pile = 1
